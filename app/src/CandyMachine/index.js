...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachine = async () => {
    ...
    console.log('CANDY IDL: ', idl)

    // Create a program that you can call
    const program = new Program(idl, candyMachineProgram, provider)
    console.log('CANDY PROGRAM: ', program)
...
const mintToken = async () => {
  ...
  const userPayingAccountAddress = candyMachineState.state.tokenMint
    ? (await getAtaForMint(candyMachineState.state.tokenMint, walletAddress.publicKey))[0]
...
return (
  candyMachineState && (
    <div className="machine-container">
      <p>{`Drop Date: ${candyMachineState.state.goLiveDateTimeString}`}</p>
      <p>{`Items Minted: ${candyMachineState.state.itemsRedeemed} / ${candyMachineState.state.itemsAvailable}`}</p>
